...
const virat = PlayerBuilder
  ...
  for (let i = 6; i <= 7; i++) {
    const hardik = PlayerBuilder
    .allRounder()
    .named("Hardik Pandya")
    .build();
  
      const allrounder = new TeamSelection(
        hardik,
        i,
        false,
        false,
  ...
  for (let i = 8; i <= 11; i++) {
      ...
      const bowler = new TeamSelection(
        bumrah,
        i,
        false,
        false,
...
const virat = PlayerBuilder
  ...
  const extraSelection = new TeamSelection(
    virat,
    12,
    false,
    false,
...
const virat1 = PlayerBuilder
  .batter()
  .named("Virat Kohli")
  .build();

const virat2 = PlayerBuilder
  .batter()
  .named("Virat Kohli")
  .build();

  team.addSelection(
    new TeamSelection(virat1, 3),
  );

  // Act
  const result = team.addSelection(
    new TeamSelection(virat2, 3),
  );
