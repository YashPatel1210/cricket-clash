...
describe("DefaultOutcomeDistribution", () => {
  it("should create the default T20 distribution", () => {
    ...
    expect(distribution.getWeightFor(DeliveryOutcome.DOT)?.getWeight()).toBe(
      32,
    );

    expect(distribution.getWeightFor(DeliveryOutcome.ONE)?.getWeight()).toBe(
      28,
    );
    expect(distribution.getWeightFor(DeliveryOutcome.TWO)?.getWeight()).toBe(
      10,
    );
    expect(distribution.getWeightFor(DeliveryOutcome.THREE)?.getWeight()).toBe(
      1,
    );
    ...
    expect(distribution.getWeightFor(DeliveryOutcome.WICKET)?.getWeight()).toBe(
      7,
    );
  });
});
